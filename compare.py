...
def save_deletion_sequences(reads, outputs, context, read_name, indel_name, threshold = 0):
    """
    indel_name - used in ID in output; "deletion" or "insertion",
                 but anything else will work too.
    threshold - min distance from start or end of the sequence
    """
    query, subject = reads
...
def analyse_basecalling(basecalling_directory, true_directory,
    ...
    sys.stdout.write("%d reads to parse\n" % len(basecalled_reads))
    counter = 0
    ...
    for read_name in basecalled_reads:
        ...
        counter += 1
        if counter % 500 == 0:
            sys.stdout.write("%d reads parsed\n" % counter)
        # DO TESTOW:
        #if counter > 50:
        #    break
...
def argument_parsing():
    ...
    # TODO: make this constant depend on the length of an indel, maybe?
...
def main():
    ...
    output.close()
    sys.stdout.write("%d skipped\n" % SKIPPED)
    sys.stdout.write("%d trailing skipped\n" % SKIPPED_TRAILING)
    sys.stdout.write("Summary written to %s, enjoy.\n" % arguments.output)
